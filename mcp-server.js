...
const NEEDS_KEY = 'AE_API_KEY is not set. This is a vault-governed tool; set the vault-issued API key to use it. (Sovereign verification needs no key — use ae_verify_sovereign.)';
...
server.registerTool(
  ...
  async ({ agentId, actionIndex, payload, signature, expectedActionEnvelopeHash }) => {
    ...
    try {
      return ok(
        await client.verifyAction({ agentId, actionIndex, payload, signature, expectedActionEnvelopeHash }),
      );
    } catch (err) {
      return fail(err instanceof Error ? err.message : 'verification failed');
    }
